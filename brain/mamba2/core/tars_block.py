...
Один слой нейронной сети TARS.
TarsCoreBlock (ssd.py): Mamba-2 SSD + RWKV-7 GatedDeltaNet + SwiGLU Fusion.
TarsBlock добавляет:     MoLE + RAG injection + Memory injection.

Убрано (v3.1):
  - PredictiveCoding (теория биологии, не подтверждена для SSM)
  - Ω-SSM Cayley (SSD стабилен по дизайну)
  - SWA per-block (заменён SharedGlobalAttention в model.py — 1 на всю модель)

Архитектура:
  x → norm → TarsCoreBlock → +residual → RAG → MoLE → Memory → out
"""
...
class TarsBlock(nn.Module):
    """
    Единый блок TARS v3.1 (Clean Architecture).
    
    Внутри:
      1. TarsCoreBlock — SSD + RWKV-7 GatedDeltaNet + MetaTokens + SwiGLU Fusion
      2. MoLE (per-block, sparse top-2) — экспертная маршрутизация
      3. RAG injection — впрыск знаний из сжатого RWKV-состояния
      4. Fused MemoryInjector — впрыск памяти (LEANN/Titans)
    """
    ...
    def __init__(
        ...
        # ═══ 1. Deep Hybrid Core (SSD + RWKV-7 GatedDeltaNet + SwiGLU) ═══
    ...
    def forward(
        ...
        Args:
            ...
            x_prev_layer: unused (kept for API compat)
            mem_signal: [B, d_model] — pre-computed memory signal
        ...
        # ═══ h_mean: вычисляем ОДИН раз, переиспользуем ═══
        ...
        # ═══ 4. NoveltyGate (inference-only) ═══
        h_new = x.mean(dim=1)
        _, novelty = self.novelty_gate(h_mean, h_new)
        if not self.training:
            # При inference: пропускать бесполезные обновления
        ...
        self.last_stats = {
            ...
            "pred_error": 0.0,  # removed PredictiveCoding
        }
