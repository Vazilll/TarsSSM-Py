...
class ThinkingLogger:
    ...
    def __init__(self, log_dir: str = "data/thinking_logs"):
        ...
        self._broadcast_hook = None  # Callback for live dashboard
        os.makedirs(log_dir, exist_ok=True)
    
    def set_broadcast_hook(self, hook_fn):
        """
        Set a callback for live broadcasting to Thinking Dashboard.
        
        hook_fn(data: dict) — called on session_start, log_step, end_session.
        Used by ui.ws_server.ThinkingBroadcaster.
        """
        self._broadcast_hook = hook_fn
    ...
    def log_step(self, step: int, data: Dict[str, Any]):
        ...
        if self._broadcast_hook:
            try:
                self._broadcast_hook({"type": "wave_update", **entry, **data})
            except Exception:
                pass
        ...
        if self._broadcast_hook:
            try:
                self._broadcast_hook({
                    "type": "session_end",
                    "total_ms": total_ms,
                    "waves": len(self.current_session["steps"]),
                    "blocks_executed": self.current_session["matrices_used"],
                    "final_p": self.current_session["final_p"],
                    "converged": self.current_session["converged"],
                })
            except Exception:
                pass
