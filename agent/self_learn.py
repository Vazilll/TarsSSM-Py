...
class SelfLearner:
    ...
    def __init__(self, memory: Optional[LeannIndex] = None):
        self.memory = memory or LeannIndex()
        self.corpus_dir = CORPUS_DIR
        self.corpus_dir.mkdir(parents=True, exist_ok=True)
        
        # Корпусный файл для дообучения
        self.corpus_file = self.corpus_dir / "self_learned_corpus.txt"
    ...
    async def learn_topic(self, topic: str, depth: int = 1) -> dict:
        ...
        for i, result in enumerate(results, 1):
            ...
            if not content or len(content) < self.min_content_length:
                print(f"       ⚠ Мало контента ({len(content)} символов), пропуск")
                continue
    ...
    async def auto_learn(self, topics: List[str], interval_min: int = 30):
        ...
        print(f"  ⏰ Интервал: {interval_min} мин")
